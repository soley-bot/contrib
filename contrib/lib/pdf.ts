...
export function generateReport(
  ...
  doc.setFontSize(18);
  setColor(doc, GRAY_DARK);
  doc.text('Contribution Record', ML, y);

  // Course name badge (top-right, if available)
  if (courseName) {
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
    setColor(doc, GRAY_MID);
    doc.text(courseName, PW - MR, y, { align: 'right' });
  }
  y += 7;
  ...
  doc.setFontSize(9);
  setColor(doc, GRAY_MID);
  const subtitleParts = [university, 'Generated by Contrib', fmtDate(new Date().toISOString())].filter(Boolean);
  doc.text(subtitleParts.join('  ·  '), ML, y);
  y += 6;
  ...
  if (courseName) details.splice(2, 0, ['Course', courseName]);
  if (university) details.splice(courseName ? 3 : 2, 0, ['University', university]);
  ...
  if (mode === 'teacher') {
    ...
    // Prominent executive summary with colored left border
    doc.setFillColor(TCL[0], TCL[1], TCL[2]);
    doc.roundedRect(ML, y - 4.5, CW, 8, 1.5, 1.5, 'F');
    doc.setFillColor(TC[0], TC[1], TC[2]);
    ...
    doc.setFontSize(11);
    setColor(doc, GRAY_DARK);
    doc.text('Executive Summary', ML + 6, y + 0.5);
    y += 8;
  ...
  if (sorted.length > 0) {
    ...
    const activeDays = new Set(sorted.map((e) => new Date(e.created_at).toDateString())).size;
    const uniqueActors = new Set(sorted.map((e) => e.actor_id)).size;

    doc.setFillColor(TCL[0], TCL[1], TCL[2]);
    doc.roundedRect(ML, y, CW, 10, 1.5, 1.5, 'F');

    const tlStats = [
      { label: 'First activity', value: fmtDate(firstDate.toISOString()) },
      { label: 'Last activity',  value: fmtDate(lastDate.toISOString()) },
      { label: 'Total entries',  value: String(sorted.length) },
      { label: 'Active days',    value: String(activeDays) },
      { label: 'Contributors',   value: String(uniqueActors) },
    ];
    const tlW = CW / tlStats.length;
    tlStats.forEach((s, i) => {
      const sx = ML + i * tlW + tlW / 2;
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(9);
      doc.setTextColor(TC[0], TC[1], TC[2]);
      doc.text(s.value, sx, y + 4.5, { align: 'center' });
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(6.5);
      setColor(doc, GRAY_MID);
      doc.text(s.label, sx, y + 8, { align: 'center' });
    });
    y += 14;
  }
  ...
  if (evaluationSummaries.length > 0) {
    ...
    const allComments = mode === 'teacher' ? evaluationSummaries.flatMap((s) => s.comments ?? []) : [];
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    const footerLeft = `${group.name}${courseName ? `  ·  ${courseName}` : ''}`;
    const footerRight = `Generated by Contrib  ·  Page ${i} of ${pageCount}`;
    doc.text(footerLeft, ML + 2, PH - 3);
    doc.text(footerRight, PW - MR - 2, PH - 3, { align: 'right' });
  }
