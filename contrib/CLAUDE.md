...
- Phase 2 cache work (remaining hook migrations): SWR is adopted; `useUser`/`ProfileProvider`, `useGroups`, `useTasks`, `useActivity`, `useDashboardSummary` are migrated (plan: `docs/superpowers/plans/2026-04-06-cache-and-cold-start.md`). The other ~18 hooks in `hooks/` still use `useState + useEffect + fetch`. Migrate them when user count >50 or when a new page's fetch waterfall becomes noticeable.
