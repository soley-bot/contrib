...
async function fetchDashboardKpi(userId: string): Promise<{ tasksToday: number; overdue: number; logsThisWeek: number }> {
  ...
  if (activeGroupIds.length === 0) {
    ...
    return { tasksToday: 0, overdue: 0, logsThisWeek: evRes.count ?? 0 };
  }
  ...
  return {
    tasksToday: todayRes.count ?? 0,
    overdue: overdueRes.count ?? 0,
    logsThisWeek: logsRes.count ?? 0,
  };
}

export function useDashboardKpi(userId: string | undefined): DashboardKpi {
  const { data, isLoading } = useSWR(
    userId ? ['dashboard-kpi', userId] : null,
    ([, uid]: [string, string]) => fetchDashboardKpi(uid),
  );

  return {
    tasksToday: data?.tasksToday ?? 0,
    overdue: data?.overdue ?? 0,
    logsThisWeek: data?.logsThisWeek ?? 0,
    loading: isLoading,
  };
}
