...
const STATUS_BADGE: Record<string, string> = {
  ...
  inprogress: 'bg-[#FEF3C7] text-amber',
  done: 'bg-[#DCFCE7] text-green',
};
const LEFT_BORDER: Record<string, string> = {
  todo: 'border-l-brand-border',
  inprogress: 'border-l-amber',
  done: 'border-l-green',
};
...
export default function TaskCard({ task, isLead, currentUserId, evidenceCount = 0, onClick, onEdit, onDelete }: TaskCardProps) {
  ...
  return (
    <div
      ...
      className={`group ${cardBg} border border-border border-l-4 ${LEFT_BORDER[task.status]} rounded-xl px-3.5 pt-3.5 pb-3.5 pl-[18px] mb-2.5 cursor-pointer active:shadow-md transition-all`}
      style={isDone ? { animation: 'done-flash 0.6s ease-out forwards' } : undefined}
    >
      <div className="flex items-start justify-between gap-2">
        <p className="text-[14px] font-medium text-text mb-2 flex-1">{task.title}</p>
        <div className="flex items-center gap-0.5 flex-shrink-0 -mt-0.5 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
          {canEdit && (
            <button
              ...
              className="p-2.5 -m-1 rounded-md text-text-tertiary hover:text-text-secondary hover:bg-bg-hover transition-colors"
              title="Edit task"
          ...
          {isLead && (
            <button
              ...
              className="p-2.5 -m-1 rounded-md text-text-tertiary hover:text-red-500 hover:bg-red-50 transition-colors"
              title="Delete task"
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {evidenceCount > 0 && (
          <span className="text-[11px] font-medium text-green">Proof ({evidenceCount})</span>
        )}
        {task.status === 'done' && evidenceCount === 0 && (
          <span className="text-[11px] text-text-tertiary">No proof</span>
        )}
