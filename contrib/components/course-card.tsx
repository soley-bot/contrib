...
export default function CourseCard({ course, groupCount, memberCount, onClick, onEdit, onDelete }: CourseCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className="bg-white border border-border rounded-xl p-4 flex items-center gap-3.5 cursor-pointer hover:shadow-md transition-shadow shadow-sm"
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[15px] font-semibold text-text truncate">{course.name}</p>
      ...
      {(onEdit || onDelete) ? (
        <div className="relative flex-shrink-0" ref={menuRef} onClick={(e) => e.stopPropagation()}>
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-md text-text-tertiary hover:text-text-secondary hover:bg-bg-hover transition-colors"
          >
            ...
            <div className="absolute right-0 top-8 w-36 bg-white border border-border rounded-[8px] shadow-lg py-1 z-20"
              ...
              {onEdit && (
                <button
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2 text-[13px] font-medium text-text-secondary hover:bg-bg-hover transition-colors"
                >
                  Edit course
                </button>
              )}
