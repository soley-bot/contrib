    ...
    <div className="max-w-2xl mx-auto px-4 py-4 pb-24 md:pb-4">
      ...
      <div className="bg-white border border-[#E2E8F0] rounded-xl p-4 mb-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <span className="w-5 h-5 rounded-full bg-[#DCFCE7] text-[#16A34A] flex items-center justify-center"><IconCheck size={12} /></span>
            <span className="text-sm font-semibold text-[#0F172A]">Submitted</span>
          </div>
      ...
      {mySummary && (
        <div className="mb-4">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-[#94A3B8] mb-2">Your scores</p>
          <div className="bg-white border border-[#E2E8F0] rounded-xl p-4">
            <div className="mb-3">
              <p className="text-[12px] text-[#475569] mb-1.5">Contribution</p>
              <ScoreBar value={Number(mySummary.avg_contribution)} />
            </div>
            <div className="mb-3">
              <p className="text-[12px] text-[#475569] mb-1.5">Collaboration</p>
              <ScoreBar value={Number(mySummary.avg_collaboration)} />
            </div>
            <p className="text-[11px] text-[#94A3B8]">Based on {mySummary.eval_count} response{mySummary.eval_count !== 1 ? 's' : ''}</p>
            {mySummary.comments && mySummary.comments.length > 0 && (
              <div className="mt-3 pt-3 border-t border-[#E2E8F0]">
                <p className="text-[11px] font-semibold uppercase tracking-wider text-[#94A3B8] mb-2">Comments received (anonymous)</p>
                {mySummary.comments.map((c, i) => (
                  <p key={i} className="text-sm text-[#475569] italic mb-1.5">"{c}"</p>
                ))}
              </div>
            )}
          </div>
      ...
      {/* All members */}
      <p className="text-[11px] font-semibold uppercase tracking-wider text-[#94A3B8] mb-2">All members</p>
            ...
            <div key={m.profile_id} className={`flex items-center gap-3 px-4 py-3 ${idx < members.length - 1 ? 'border-b border-[#E2E8F0]' : ''}`}>
              ...
              {s ? (
                <div className="flex gap-3 text-[12px] flex-shrink-0">
                  <span className="text-[#475569]">C: <span className="font-semibold text-[#0F172A]">{s.avg_contribution}</span></span>
                  <span className="text-[#475569]">Col: <span className="font-semibold text-[#0F172A]">{s.avg_collaboration}</span></span>
                </div>
              ) : (
                <span className="text-[11px] text-[#94A3B8]">No data yet</span>
              )}
            </div>
          );
        })}
