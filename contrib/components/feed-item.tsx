...
const ACTION_META: Record<string, { icon: IconComponent; color: string; label: (meta: Record<string, unknown> | null) => string }> = {
  ...
  file_uploaded:        { icon: IconClip,     color: 'var(--brand)', label: (m) => `logged work for "${m?.task_title ?? ''}"` },
  evidence_added:       { icon: IconClip,     color: 'var(--brand)', label: (m) => `logged work for "${m?.task_title ?? ''}"` },
  evidence_version_added: { icon: IconClip,   color: 'var(--brand)', label: (m) => `added new version for "${m?.task_title ?? ''}"` },
