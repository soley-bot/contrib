...
export default function JoinPage() {
  ...
  if (status === 'loading' || userLoading) {
    return <div className="flex items-center justify-center min-h-dvh text-[#64748B]">Loading…</div>;
  }
  ...
  if (status === 'error') {
    return (
      <div className="flex flex-col items-center justify-center min-h-dvh gap-3 px-5 text-center">
        ...
        <p className="text-sm text-[#64748B]">{joinError}</p>
        <button onClick={() => router.push('/dashboard')} className="mt-2 text-[#1A56E8] text-sm font-medium">
          Back to dashboard
        </button>
      </div>
    );
  ...
  if (status === 'full') {
    return (
      <div className="flex flex-col items-center justify-center min-h-dvh gap-3 px-5 text-center">
        <svg width="48" height="48" viewBox="0 0 48 48" fill="none" className="text-[#94A3B8]">
          <circle cx="24" cy="24" r="22" stroke="currentColor" strokeWidth="2"/>
          <path d="M16 24h16" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
        </svg>
        <p className="text-lg font-semibold text-[#0F172A]">This group is full</p>
        <p className="text-sm text-[#64748B]">This group already has 6 members and cannot accept new members.</p>
  ...
  if (status === 'joined') {
    return (
      <div className="flex flex-col items-center justify-center min-h-dvh gap-3">
        <span className="text-[#16A34A]"><IconCheck size={32} /></span>
        <p className="text-lg font-semibold text-[#16A34A]">Joined! Redirecting…</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-dvh px-5">
      <div className="bg-white border border-[#E2E8F0] rounded-xl p-6 max-w-sm w-full text-center shadow-sm">
        ...
        <h1 className="text-lg font-bold text-[#0F172A] mb-1">{group?.name}</h1>
        <p className="text-sm text-[#94A3B8] mb-6">{group?.subject}</p>
        <button
          onClick={handleJoin}
