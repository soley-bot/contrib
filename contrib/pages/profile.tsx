  ...
  return (
    <div className="min-h-dvh bg-[#FAFAF9]">
      ...
      <div className="md:pl-[220px]">
        <div className="hidden md:flex items-center h-14 px-6 bg-white border-b border-[#E7E5E4]">
          <span className="text-base font-semibold text-[#1C1917]">Profile</span>
        </div>

        <div className="pt-14 md:pt-0 px-4 py-6 max-w-lg mx-auto">
          {/* Profile card */}
          <div className="bg-white border border-[#E7E5E4] rounded-[12px] p-6" style={{ boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
            {/* Avatar + name row */}
            <div className="flex items-center gap-4 mb-5">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xl font-bold text-[#1C1917] truncate">{profile.name}</p>
                <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                  {profile.university && <span className="text-sm text-[#57534E]">{profile.university}</span>}
                  {profile.faculty && <><span className="text-[#D6D3D1]">·</span><span className="text-sm text-[#57534E]">{profile.faculty}</span></>}
                  {profile.year_of_study && <><span className="text-[#D6D3D1]">·</span><span className="text-sm text-[#57534E]">{profile.year_of_study}</span></>}
                </div>
                <span className={`inline-block mt-1.5 px-2 py-0.5 rounded text-[11px] font-semibold uppercase tracking-wide ${
                  profile.role === 'teacher' ? 'bg-[#EFF6FF] text-[#1D4ED8]' : 'bg-[#F0FDF4] text-[#15803D]'
                }`}>
                  {profile.role}
                </span>
              </div>
            ...
            {/* Edit form */}
            {editing ? (
              <div className="flex flex-col gap-3.5 border-t border-[#E7E5E4] pt-4">
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-[13px] font-medium text-[#57534E]">University <span className="font-normal text-[#A8A29E]">(optional)</span></label>
                ...
                <div className="flex gap-2 pt-1 border-t border-[#E7E5E4]">
              ...
              <button onClick={() => setEditing(true)}
                className="w-full h-10 border border-[#E7E5E4] bg-[#FAFAF9] hover:bg-[#F5F5F4] text-[#57534E] text-sm font-medium rounded-md transition-colors">
                Edit profile
              </button>
            )}
          </div>
          ...
          {isStudent && (
            <div className="mt-4">
              <p className="text-[11px] font-semibold uppercase tracking-wider text-[#A8A29E] mb-2.5">Your stats</p>
              <div className="grid grid-cols-3 gap-2.5">
                {[
                  ...
                  <div key={s.label} className="bg-white border border-[#E7E5E4] rounded-[10px] px-3 py-3 text-center"
                    style={{ boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
                    <p className="text-2xl font-bold text-[#1C1917]">{s.value}</p>
                    <p className="text-[11px] text-[#A8A29E] mt-0.5 leading-tight">{s.label}</p>
                  </div>
                ))}
              </div>
            </div>
